...
class ChickTimerStatus:
    ...
    def getValues(self):
        return [
            self.DaysSinceStatusChanged,
            self.DaysSinceHatch,
            self.DaysSinceDesertionTriggered,
            self.TimeOfEmergence,
            self.WeeksOfBatteryLeft,
            self.ActivityYesterday,
            self.ActivityTwoDaysAgo,
            self.MeanActivity,
        ]
...
class ChickTimer:
    ...
    def __init__(self):
        ...
        self.dbfs = DecibelsFullScale(min=0.0, max=0.0, mean=0.0)
        self.snr = SignalToNoiseRatio(min=0.0, max=0.0, mean=0.0)
        ...
        return json.dumps(
            {
                ...
                "channel": self.channel,
                "snr": {"min": self.snr.min, "max": self.snr.max, "mean": self.snr.mean},
                "dbfs": {"min": self.dbfs.min, "max": self.dbfs.max, "mean": self.dbfs.mean},
                "lat": self.lat,
                "lon": self.lon,
