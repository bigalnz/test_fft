...
clibrary.airspyhf_lib_version.argtypes = [ctypes.POINTER(VER)]  # declare the method types for the params using the struct
...
def get_sample_rates(device_handle):
    ...
    status = clibrary.airspyhf_get_samplerates(device_handle, rates.ctypes.data_as(ctypes.POINTER(ctypes.c_uint32)), ctypes.c_uint32(num_rates) )
...
def rx_callback(transfer):
    ...
    complex_data = numpy.ctypeslib.as_array(transfer.contents.samples, shape=(transfer.contents.samples_count,2)).view('complex64')
    print(complex_data)
    return 0
